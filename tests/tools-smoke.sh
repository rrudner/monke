...
test_root=$(mktemp -d /tmp/monke-tools.XXXXXX)
...
state=$test_root/home/.config/monke/tools.state
[[ $(wc -l <"$state") -eq 8 ]]
awk -F '\t' '$2 != "system" {exit 1}' "$state"
grep -q '`ripgrep` (`rg`): system' \
    "$test_root/home/.config/monke/capabilities.md"
grep -q 'Fast recursive text search' "$test_root/home/.config/monke/capabilities.md"
...
grep -q '`shfmt` (`shfmt`): system' \
    "$test_root/home/.config/monke/capabilities.md"
grep -qx ast-grep "$test_root/home/.config/monke/tools.catalog-reviewed"
[[ ! -e $test_root/home/.local/state/monke/tools-reconfigure-required ]]
[[ ! -e $test_root/home/.local/share/monke/runtime/mise ]]
...
! grep -qx gh "$test_root/home/.config/monke/tools.selected"
[[ -x $test_root/bin/gh ]]

HOME="$test_root/home" "$repo_dir/scripts/tools-manager.sh" clear
[[ ! -s $test_root/home/.config/monke/tools.selected ]]

local_home=$test_root/local-home
runtime_dir=$local_home/.local/share/monke/runtime
mkdir -p -- "$runtime_dir"
...
shim_dir=$local_home/.local/share/monke/mise/shims
mkdir -p -- "$shim_dir"
install_bin=$local_home/.local/share/monke/mise/installs/sops/1.2.3/bin
mkdir -p -- "$install_bin"
...
grep -q '^"npm:@playwright/cli" = "1.2.3"$' \
    "$local_home/.config/monke/mise.toml"
grep -q '^export PLAYWRIGHT_BROWSERS_PATH=.*monke/playwright' \
    "$local_home/.config/monke/tools-env.sh"
...
grep -qx lighthouse "$old_node_home/.config/monke/tools.selected"
! grep -q $'^lighthouse\t' "$old_node_home/.config/monke/tools.state"
...
mkdir -p -- "$legacy_home/.config/monke"
printf '%s\n' ripgrep fd jq yq shellcheck >"$legacy_home/.config/monke/tools.selected"
PATH="$test_root/bin:/usr/bin:/bin" HOME="$legacy_home" \
    "$repo_dir/scripts/tools-manager.sh" reconcile
[[ "$(cat "$legacy_home/.local/state/monke/tools-reconfigure-required")" == \
    $'ast-grep\nshfmt' ]]
printf '\n' | PATH="$test_root/bin:/usr/bin:/bin" HOME="$legacy_home" \
    script -qec "'$repo_dir/scripts/tools-manager.sh' configure --review ast-grep,shfmt" /dev/null \
    >"$test_root/legacy-configure.log"
[[ ! -e $legacy_home/.local/state/monke/tools-reconfigure-required ]]
grep -qx ast-grep "$legacy_home/.config/monke/tools.catalog-reviewed"
...
! grep -qx ripgrep "$legacy_home/.config/monke/tools.selected"
grep -q $'^ripgrep\tsystem\t' "$legacy_home/.config/monke/tools.state"
! grep -qx ast-grep "$legacy_home/.config/monke/tools.selected"
! grep -qx shfmt "$legacy_home/.config/monke/tools.selected"
...
cat >"$test_root/home/.local/state/monke/tools-reconfigure-required" <<'EOF'
ast-grep
EOF
PATH="$test_root/bin:/usr/bin:/bin" HOME="$test_root/home" \
    "$repo_dir/scripts/tools-manager.sh" catalog-diff "$catalog_before" "$catalog_after" 1 \
    >"$test_root/catalog-diff-merge.log"
grep -qx ast-grep "$test_root/home/.local/state/monke/tools-reconfigure-required"
grep -qx beta "$test_root/home/.local/state/monke/tools-reconfigure-required"
grep -qx gamma "$test_root/home/.local/state/monke/tools-reconfigure-required"
[[ "$(cat "$test_root/home/.local/state/monke/tools-reconfigure-required")" == $'ast-grep\nbeta\ngamma' ]]
