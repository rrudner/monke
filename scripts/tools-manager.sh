...
config_root=${XDG_CONFIG_HOME:-"$HOME/.config"}/monke
data_root=${XDG_DATA_HOME:-"$HOME/.local/share"}/monke
cache_root=${XDG_CACHE_HOME:-"$HOME/.cache"}/monke
state_root=${XDG_STATE_HOME:-"$HOME/.local/state"}/monke
...
write_runtime_files() {
    ...
    printf '[tools]\n' >"$config_tmp"
    printf '%s\n' '# Generated by Monke. Do not source outside monke.' >"$env_tmp"
    printf 'export MONKE_ACTIVE=1\n' >>"$env_tmp"
    printf 'export MONKE_TOOLS=' >>"$env_tmp"
    printf '%s\n' '""' >>"$env_tmp"
    printf '%s\n' '### Available Monke tools' >"$capabilities_tmp"
...
check_updates() {
    ...
    [[ -z $output ]] || printf 'monke: local tool updates are available; run monke tools update\n'
}
...
remove_tools() {
    ...
    [[ -n $names ]] || { printf 'Usage: monke tools remove NAME[,NAME...]\n' >&2; return 2; }
